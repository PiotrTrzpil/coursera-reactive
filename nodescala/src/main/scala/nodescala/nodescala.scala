...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val subscriptions = new java.util.concurrent.ConcurrentHashMap[Subscription,Unit]()

    def loop(token: CancellationToken): Unit = {

        val subscr = Future.run() { (token )=>
          listener.nextRequest().map { case (req, exchange) =>
            if(!token.isCancelled) {

              Future.any(List(Future {
                respond(exchange, token, handler(req))
              }, Future.delay(3.seconds))).continueWith { f =>
                exchange.close()
                loop(token)
              }
            }
    ...
    loop(source.cancellationToken)
    new Subscription {
      override def unsubscribe(): Unit = {
        source.unsubscribe()

        listenerSub.unsubscribe()
        subscriptions.keys().foreach(_.unsubscribe())
      }
    }
  }
