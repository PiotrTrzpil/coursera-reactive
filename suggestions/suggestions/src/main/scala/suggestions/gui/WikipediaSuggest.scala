...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestions: Observable[Try[List[String]]] =
      searchTerms.flatMap(term => Observable.from(wikipediaSuggestion(term)).recovered)
    ...
    val selections: Observable[String] =
      button.clicks.filter(_ => suggestionList.listData.nonEmpty)
        .map(_ => suggestionList.selection.items.self.head)

    // TO IMPLEMENT
    val pages: Observable[Try[String]] =
      selections.flatMap(sel => Observable.from(wikipediaPage(sel)).recovered)
