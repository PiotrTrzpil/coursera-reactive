...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = obs.map(Success(_)).onErrorReturn { case ex => Failure(ex)}
    ...
    def timedOut(totalSec: Long): Observable[T] =
      obs.takeUntil(Observable.never.timeout(FiniteDuration(totalSec, "seconds")).onErrorReturn(ex => ()))
